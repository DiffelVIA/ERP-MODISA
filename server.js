...
app.get('/api/pagos', async (req, res) => {
    try {
        const query = `
            ...
            LEFT JOIN contracts c ON c.id_contract = COALESCE(
                pod.id_contract,
                (
                    SELECT c_sub.id_contract 
                    FROM contracts c_sub 
                    WHERE LOWER(TRIM(c_sub.supplier)) = LOWER(TRIM(pod.provider))
                      AND (c_sub.id_project_category = pod.id_project_category OR c_sub.id_project_category IS NULL)
                    ORDER BY (c_sub.id_project_category = pod.id_project_category) DESC, c_sub.start_date DESC
                    LIMIT 1
                )
            )
