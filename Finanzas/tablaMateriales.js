...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (esResidente) {
        ...
        encabezados.forEach(th => {
            ...
            if (texto.includes('proveedor') || texto.includes('precio') || texto.includes('monto') || texto.includes('total')) {
                th.remove();
            }
        });
    }
...
function renderizarTabla(materialesAVer) {
    ...
    materialesAVer.forEach((item) => {
        ...
        if (esResidente) {
            ...
            tr.innerHTML = `
                <td style="width: 10%; font-weight: bold;">${item.solicitante || 'Sin Nombre'}</td>
                <td style="width: 12%;">${item.obra || 'Sin Obra'}</td>
                <td style="width: 10%;">${formatearFecha(item.order_date)}</td>
                <td style="width: 8%; color: var(--text-muted); text-align: center;">${item.fiscal_week}</td>
                <td style="width: 10%;">${item.grupo || 'General'}</td>
                <td style="width: 10%;">${item.categoria || 'General'}</td>
                <td style="width: 12%;">${item.subcategoria || 'General'}</td>
                <td style="width: 18%; font-weight: 500;">${item.material_description}</td>
                <td style="width: 5%; text-align: center;">${item.unit}</td>
                <td style="width: 5%; text-align: center; font-weight: bold;">${item.quantity}</td>
                <td style="width: 20%; color: var(--text-muted); font-style: italic;">${item.commentary || '-'}</td>
